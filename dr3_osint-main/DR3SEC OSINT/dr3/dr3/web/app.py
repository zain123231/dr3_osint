...
import dr3
import dr3.settings
from dr3.sites import MaigretDatabase
from dr3.report import generate_report_context
...
# Configuration
MAIGRET_DB_FILE = os.path.join('dr3', 'resources', 'data.json')
...
async def maigret_search(username, options):
    logger = setup_logger(logging.WARNING, 'dr3')
    try:
...
def process_search_task(usernames, options, timestamp):
    try:
        ...
        for username, id_type, results in general_results:
            ...
            dr3.report.save_csv_report(csv_path, username, results)
            dr3.report.save_json_report(
                json_path, username, results, report_type='ndjson'
            )
            dr3.report.save_pdf_report(pdf_path, context)
            dr3.report.save_html_report(html_path, context)
            ...
            for site_name, site_data in results.items():
                if (
                    ...
                    == dr3.result.MaigretCheckStatus.CLAIMED
                ):
